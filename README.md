...
ipconfig /all — Full IP configuration<br>ipconfig /release — Release IPv4 address<br>ipconfig /renew — Renew IPv4 address<br>ping <IPaddress> — Test connection (User Input)<br>tracert <hostname> — Trace route (User Input)<br>netstat -ano — Active connections with PIDs

DNS Fixes

Dropdown Menu

ipconfig /flushdns — Clear DNS resolver cache<br>ipconfig /displaydns — View DNS resolver cache<br>nslookup <domain> — Query DNS for domain (User Input)<br>nslookup <domain> <dns_server> — Query specific server (User Input)

System + Files

Dropdown Menu

systeminfo — Detailed system configuration<br>whoami /all — Current user and privileges<br>hostname — Display computer name<br>sfc /scannow — Scan and repair system files<br>DISM ... /RestoreHealth — Repair Windows system image<br>robocopy <src> <dst> — Mirror directory tree (User Input)

Disk + Storage

Dropdown Menu

chkdsk c: /scan — Scan local volume for errors<br>wmic logicaldisk get ... — View disk capacity & free space<br>diskpart — Interactive disk, partition, and volume manager

Processes + Services

Dropdown Menu

tasklist — View running tasks & PIDs<br>taskkill /PID <id> /F — Force terminate process (User Input)<br>sc query — Enumerate active services<br>sc stop <service> — Stop a running service (User Input)<br>sc start <service> — Start a configured service (User Input)

Remote + Shares

Dropdown Menu

mstsc — Launch Remote Desktop Connection client<br>ssh <user@host> — Connect via Secure Shell (User Input)<br>net use \\\\<pc>\\C$ — Map remote admin share (User Input)<br>net use * /delete — Disconnect all network mapped drives

Extra Goodies

Dropdown Menu

cls — Clear screen command sequence<br>echo %PATH% — Display the environment path variable<br>ver — Display detailed Windows OS version<br>driverquery — Display installed kernel device drivers<br>getmac — Display MAC addresses of active network adapters<br>arp -a — View Address Resolution Protocol cache table<br>ipconfig /registerdns — Refresh DHCP leases and register DNS names<br>powercfg /batteryreport — Generate detailed battery health report<br>shutdown /s /t 0 — Shutdown the computer immediately

Security Artifacts

Interactive Tool

Check presence, read forensic significance analysis, and launch File Explorer focusing on critical directories (SAM, Prefetch, Amcache.hve, NTUSER.dat, hosts, etc.).

Help

Scrollable Box

Core requirements and a comprehensive guide to building/compiling installer setups with troubleshooting for standard error warnings (0x80004003).
...
Open your terminal (Command Prompt or PowerShell) and navigate to your target coding directory.

Run the following command to scaffold a new Windows Forms application:
...
Replace the default boilerplate in Form1.cs with the production-ready code.

Run the application directly from the CLI:
...
Search for Windows Forms App, select it using C#, and click Next.

Set the project name to CMDarsenal, choose .NET 6.0, .NET 7.0, or .NET 8.0 (Long-Term Support), and click Create.

In Solution Explorer, double-click Form1.cs to open its code editor.

Delete all existing code inside Form1.cs and paste the updated programmatic, dark-themed code.

Press F5 or click the green Start/Debug button to build and run.
...
Ensure your dropdown string in Step 1 includes a placeholder, such as ping <IPaddress>.

Inside RunCommandInTerminal, find the switch (commandToRun) statement and add a new case for your isolated command string.

Call ShowInputDialog to prompt the user, verify they did not select Cancel, and construct the final command string.
...
Open the File System Editor in your Setup Project.

Select Application Folder, press F4 to open Properties, and set AlwaysCreate to True.

Repeat this process to change AlwaysCreate to True for the User's Desktop and User's Programs Menu folders.
...
Open the File System Editor of your setup project, delete the old Primary Output entry.

Right-click the folder > Add > Project Output.

Select Primary Output and set the Configuration dropdown strictly to (Active) instead of hardcoded configurations.

Clear your build cache (delete the hidden .vs folder as well as local bin and obj directories) and perform a Rebuild Solution under your target configuration (Debug/Release).
